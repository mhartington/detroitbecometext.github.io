...
test('Chapter 34 should have ids defined for all headings', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(34);
	...
	test('Chapter 34 should have labelled sections', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(34);
	...
	test('Chapter 34 should not have nested paragraphs', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(34);
	...
	test('Chapter 34 should have section for every heading', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(34);
	...
	test('Chapter 34 should have a condition group for each condition content', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(34);
