...
test('Chapter 39 should have ids defined for all headings', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(39);
	...
	test('Chapter 39 should have labelled sections', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(39);
	...
	test('Chapter 39 should not have nested paragraphs', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(39);
	...
	test('Chapter 39 should have section for every heading', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(39);
	...
	test('Chapter 39 should have a condition group for each condition content', async ({
		page,
	}) => {
		const currentPage = new NavigableItemPage('chapters', page);
		await currentPage.goToId(39);
